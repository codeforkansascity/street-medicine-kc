...
class Agencies {
   	...
   	public $subCatLinkTable = "Agency_has_subCategories";
        public $subCatTable = "subCategories";
	public $catTable = "categories";
	...
	function getAgencyDescription($agencyid) {
		...
		foreach($agencies as $agency) {
                	...
                	if($agency[address2]!='') $description.=", $agency[address2]";
                	...
                	if($agency[emergencyPhone]!='') $description.="<br><a href=\"tel:+1".preg_replace("/[^0-9]/","",$agency[emergencyPhone])."\">".$agency[emergencyPhone]."</a> (Emergency)";
                	$description.="</p>";
                	if($agency[Website]!='')
                        	$description.="<p><a href=\"$agency[Website]\" target=\"_blank\">Website <i class=\"fa fa-external-link\" aria-hidden=\"true\"></i></a></p>";
                	if($agency[hours]!='') $description.="<p>Hours: $agency[hours]</p>";
                	if($agency[description]!='') $description.="<p>$agency[description]</p>";
	...
	*/
	function fetchActivatedAgencySubCategories($agencyid = 0, $categoryid = 0, $orderby = "subCategory") {
		...
		$sql="SELECT t1.* FROM ".$this->subCatTable." AS t1, ".$this->subCatLinkTable." AS t2 WHERE t1.id=t2.subCategories_id AND t2.Agency_id='$agencyid'";
	 	if($categoryid>0) $sql.=" AND t1.categories_id='$categoryid'";
	   	$sql.=" ORDER BY $orderby";
                $db = new Db();
                $activatedSubCats=$db->select($sql);

                if(mysql_error()) {
                        echo "<br>MySQL Error: " . $sql . "<br>" . mysql_error() . "<br>";
                        return FALSE;
                }
	...
	*/
	function agencyHasSubCategory($agencyid,$subcatid) {
		if(!$agencyid || !$subcatid) return FALSE;
	...
	*/
	function insertAgency($name, $description, $address1, $address2, $city, $state, $zip,
        $phone, $emergencyPhone, $fax, $website, $hours, $contactFirst, $contactLast, $email, $free) {

        	$sql = "INSERT INTO ".$this->table." (`name`, `description`, `address1`, `address2`, `city`, `state`, `zip`, `phone`, `emergencyPhone`, `fax`, `website`, `hours`, `contactFirst`, `contactLast`, `email`, `free`) VALUES ('$agency', '$description' , '$address1', '$address2', '$city', '$state', '$zip', '$phone',     '$emergencyPhone', '$fax', '$website', '$hours', '$contactFirst', '$contactLast', '$email', $free)";
		$result=mysql_query($sql);
	...
	*/
	function update_agency($id, $agency, $description, $address1, $address2, $city, $state, $zip,
        $phone, $emergencyPhone, $fax, $website, $hours, $contactFirst, $contactLast, $email, $free) {
        	...
        	contactLast='$last', email='$email', free=$free WHERE id=$id;"; //STOP 6/14/16
