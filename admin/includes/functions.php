...
function login($email, $password, $mysqli) {
	...
	if ($stmt = $mysqli->prepare("SELECT id, userName, password, salt
		...
		$stmt->bind_result($admin_id, $username, $db_password, $salt);
		$stmt->fetch();
		...
		if ($stmt->num_rows == 1) {
			...
			if (checkbrute($admin_id, $mysqli) == true) {
				...
				if ($db_password == $password) {
					...
					$now = time();
					if (!$mysqli->query("INSERT INTO loginAttempts(admin_id, time)
                                    VALUES ('$admin_id', '$now')")) {
...
function checkbrute($admin_id, $mysqli) {
	...
	// var_dump($valid_attempts);
	if ($stmt = $mysqli->prepare("SELECT time
                                  FROM loginAttempts
                                  WHERE admin_id = ? AND time > '$valid_attempts'")) {
